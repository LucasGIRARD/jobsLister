...
if (isset($_GET['type'])) {
	...
	LEFT JOIN SOURCES_has_JOBS AS ShJ ON ShJ.JOBS_id = J.id 
	LEFT JOIN SOURCES AS S ON S.id = ShJ.SOURCES_id 
	WHERE JhF.id IS NULL and expired IS NULL and filtered is null
	GROUP BY J.id
...
<body>
	<div class="pure-g">
		<div class="pure-u-1 list">
			<nav class="pure-menu pure-menu-horizontal pure-form">
				...
				<span id="hideCols" class="pure-form">
					<?php
						...
						$i = 1;
						foreach ($cols as $col) {
							echo '<input type="checkbox" name="hide-'.$col.'" id="hide-'.$col.'" data-col="'.$i.'" value="1"><label for="hide-'.$col.'">'.$col.'</label>';
							$i++;
						}
					?>					
				</span>
					...
					<tbody id="contentArea" class="clusterize-content">
						...
						foreach ($listJ as $j) {
							...
							echo '<tr id="row-' . $j['id'] . '">
								...
								<td>' . $j['salaryMax'] . '</td>
								<td>' . $j['candidats'] . '</td>
								<td>' . (!empty( $j['filtered'])?$j['filtered']:'<button class="banJob pure-button button-warning" data-jobid="' . $j['id'] . '">filter</button>') . '</td>
								<td>' . $j['postulated'] . '</td>
								<td>' . $j['refused'] . '</td>
								...
								' . (isset($j['link2']) && !empty($j['link2']) && $j['link'] != $j['link2'] ? '<br><a target="_blank" href="' . $j['link2'] . '" class="pure-menu-link">lien</a>' : '') . '</td>
